...
app.get("/", function(req, res) {
	Article.find({}, null, {sort: {created: -1}}, function(err, data) {
		if(data.length === 0) {
			res.render("placeholder", {message: "To run a news scraping search, click \"Scrape For Newest Articles\" ."});
		}
		else{
...
app.get("/saved", function(req, res) {
	Article.find({issaved: true}, null, {sort: {created: -1}}, function(err, data) {
		if(data.length === 0) {
			res.render("placeholder", {message: "You can save articles here by clicking \"Save Article\"!"});
		}
		else {
...
app.post("/search", function(req, res) {
	...
	Article.find({$text: {$search: req.body.search, $caseSensitive: false}}, null, {sort: {created: -1}}, function(err, data) {
		console.log(data);
		if (data.length === 0) {
			res.render("placeholder", {message: "The news search did not yield any results. Try another search word."});
		}
		else {
